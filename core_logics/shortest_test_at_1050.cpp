...
void visitNodes(int N, int c_row, int c_col, vector<loc_s> path, int total) {
  int i;
  count_try++;
  ...
  for (i = 0; i < 4; i++) {
    switch(i) {
    case 0:
      visitNodes(N, c_row-1, c_col, path, total);
      break;
    case 1:
      visitNodes(N, c_row, c_col+1, path, total);
      break;
    case 2:
      visitNodes(N, c_row+1, c_col, path, total);
      break;
