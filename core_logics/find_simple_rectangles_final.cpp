...
bool isEdge(int cur_row, int cur_col, int max_row, int max_col) {
  ...
  while (testnum-- > 0) {
    ...
    for (i = 0; i < rows; i++) {
      for (j = 0; j < cols; j++) {
        ...
        if (g[i][j] != 0 && isEdge(i,j, rows-1, cols-1)) {
          cout << "Found rect at " << i <<","<< j <<":width("<< m[0][i][j] << "),height(" << m[1][i][j] << ")" << endl;
        }
      }
    }
  }
